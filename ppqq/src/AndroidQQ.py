...
class AndroidQQ:
    ...
    def login(self, verifyCode=None):
        ...
        print '包头 :' +  Coder.trim('00 00 00 08 02 00 00 00 04 00')
        
        packet += Coder.num2hexstr(len(self.qqHexstr)/2+4, 4)
        packet += self.qqHexstr
        
        print 'qqHexstr :' +  Coder.num2hexstr(len(self.qqHexstr)/2+4, 4) + self.qqHexstr
        
        
        packSendLoginMessage = self.packSendLoginMessage(verifyCode)
        #TEA加密的包体
        packet += packSendLoginMessage
        
        
        print 'packSendLoginMessage :' + packSendLoginMessage
        
        #总包长
        packet = Coder.num2hexstr(len(packet)/2+4, 4) + packet
        
        
        print 'Coder.num2hexstr(len(packet)/2+4, 4): ' + Coder.num2hexstr(len(packet)/2+4, 4)
